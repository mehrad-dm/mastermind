...
The goal: the user names a field (or points at a project) and gets a strong, tailored pack — the same bar
as `engineering/fields/frontend/` — from one command.
...
3. **Research + write it to the frontend bar.** Do the verified sweep (best repos/courses/people/docs, each
   ...
   *non-obvious* decisions (not what the model already knows), grounded in primary sources. Match the depth
   and density of `engineering/fields/frontend/stack-defaults.md`. Start `lessons.md` empty.
