...
The single "do it the genius way, end to end" workflow. It runs the loop from `core/agent-loop.md`,
pulling in the specialist agents and the field pack at each phase. Task: **$ARGUMENTS**.

Scale effort to the task (`core/principles.md`): a trivial change skips straight to implement+verify;
...
1. **Understand** — restate the real problem and its scope/lifespan; do the asked task and nothing more
   (`core/rigor.md` → Stay in scope). Read the relevant existing code and conventions first (delegate
   wide reading to a subagent to protect context); learn the stack if unfamiliar (`core/agent-loop.md`
...
4. **Implement to rigor** (`core/rigor.md`) — build against the design. Handle the unhappy paths
...
5. **Verify — close the loop** (`core/agent-loop.md`) — give the work a check that returns pass/fail
...
approach is wrong or unsafe, say so once with the better option (`core/rigor.md` refuse-list), then proceed.
